...
Most of these functions call G_debug() internally, which needs GISRC set
in the process environment even though none of them read or write a
project or mapset; see conftest.py's grass_session_env fixture for why
and how. Since that fixture changes os.environ, every test using it is
marked needs_solo_run so it does not affect tests running in parallel
next to it.
...
@pytest.mark.needs_solo_run
def test_assign_puts_each_point_in_its_nearest_class(grass_session_env) -> None:
    cluster = cluster_with_points(SAMPLE_POINTS, 3)
...
@pytest.mark.needs_solo_run
def test_reassign_leaves_already_nearest_points_alone(grass_session_env) -> None:
...
@pytest.mark.needs_solo_run
def test_reassign_moves_a_point_to_a_nearer_class(grass_session_env) -> None:
...
@pytest.mark.needs_solo_run
def test_sum2_accumulates_the_sum_of_squares_per_class(grass_session_env) -> None:
...
@pytest.mark.needs_solo_run
def test_merge_combines_two_classes(grass_session_env) -> None:
    cluster = cluster_with_points(SAMPLE_POINTS, 3)
    classify(cluster)
    libcluster.I_cluster_sum2(byref(cluster))
